...
import { Ilogin, User } from "../../types/user.js";
...
function App() {
  ...
  const [savedMovies, setSavedMovies] = useState<any[]>(JSON.parse(localStorage.getItem("savedMovies")!) || []);
  ...
  async function handleUpdateUser(userInfo: User) {
    try {
  ...
  async function handleSaveMovie(movie: Movie) {
    try {
  ...
  function removeMovieFromPage(movieId: number) {
  ...
  return isInit ? (
    <div className="page">
      <Header
        ...
        onBurgerMenuClick={() => setIsBurgerMenuOpen(true)}
        loggedIn={loggedIn}
      />
      <CurrentUserContext.Provider value={currentUser ?? undefined}>
        <Routes>
          <Route
            path="/movies"
            element={
          ...
          <Route
            ...
            element={
              <ProtectedRouteElement loggedIn={loggedIn} isBlockedForAuthUser={true}>
                ...
                {/* TODO <Login handleLogin={handleLogin} isBlockedForAuthUser={true} />  */}
              </ProtectedRouteElement>
            }
          />
